...
var correctDataAgainstLocCode = [];
var invPerValueSplit;
var successCount = 0;
var errorCount = 0;
...
document.getElementById('customFile').addEventListener('change', function (event) {
    if ($("#invPeriodwiseLocation").val() == "0") {
        swal({
            title: "Error",
            text: "Please select the Location first.",
            icon: "error",
            button: "OK",
            //dangerMode: true,
        });
            ...
            var results = Papa.parse(content, {
                ...
                complete: function (results) {
                    ...
                    var data = originalData.map((row, rowIndex) => {
                        ...
                        let lastValue = row.at(-1);
                        if (lastValue != invPerValueSplit) {
                            obj['Status'] = '<span style="color: red; font-weight: bold;">Error</span>';
                            errorCount += 1;
                            $("#errorCount").text(errorCount)
                        } else {
                            obj['Status'] = '<span style="color: green; font-weight: bold;">Success</span>';
                            successCount += 1;
                            $("#successCount").text(successCount);

                            correctDataAgainstLocCode.push(obj);
                        }
                        return obj;
                    });

                    correctDataAgainstLocCode = correctDataAgainstLocCode.map(record => {
                        const { ['Sr No']: _, Status, ...rest } = record;
                        return rest;
                    });
        ...
        if (invPerLocationSelection = 0) {
            ...
            swal({
                ...
                if (isConfirm) {
                    $('body').css('opacity', '0.5');
                    $('.loader').show();
                    var filteredData = correctDataAgainstLocCode
                        .map(obj => Object.values(obj))
                        .filter(row => !row.some(param => param === undefined));
...
$("#invPeriodwiseLocation").change(function () {
    ...
    if ($("#invPeriodwiseLocation").val() == "0") {
        ...
        //var invPerValue = $("#invPeriodwiseLocation option:selected").text();
        invPerValueSplit = $("#invPeriodwiseLocation option:selected").text().split("-")[0].trim();
        disableBTN.disabled = true;
        filldropdownInventory()
    }
})
