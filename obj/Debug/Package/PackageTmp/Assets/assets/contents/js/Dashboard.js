...
function FillGridItemVariance(response) {
    ...
    if (response.returnTable.length == 0) {
        ...
        for (i = 0; i < response.returnTable.length; i++) {
            $('#itemsVariance').append('<tr><td><div style="font-size: 12px;" class="d-flex align-items-center fw-medium">' + response.returnTable[i].description + '</div></td><td style="padding-right: 8px;">' + response.returnTable[i].systemData + '</td><td style="padding-right: 20px;">' + response.returnTable[i].physicalData + '</td><td>' + parseFloat(response.returnTable[i].varianceData).toFixed(2) + '</td></tr>');
...
function loadinvPeriodDD() {
    $.ajax({
        url: $('#url_local').val() + "/api/Dashboard/GetAllInvPeriodsDesc",
        type: 'POST',
        contentType: 'application/json', // Set the content type based on your API requirements
        data: JSON.stringify({ }), // Adjust the payload format based on your API
        headers: {
...
function invPeriodsddlHandler(response) {
    fillinvPeriodsddls('invPeriodDD', response)
};

function fillinvPeriodsddls(name, data) {
    $("#" + name).empty();
    var s = '';
    $.each(data, function (index, item) {
